...
pub async fn sync_problem_files<'a>(
    problem_id: i64,
        ...
        .map_err(|e| anyhow!("Failed to open data dir: {}", e))?
    {
        ...
        if !file_name.ends_with(".lock") {
            if entry
                .file_type()
                .map_err(|e| anyhow!("Failed to get filetype for {:?}: {}", entry.file_name(), e))?
                .is_file()
            {
    ...
    for file in files.into_iter() {
        ...
        if should_download {
            ...
            let data = http_client
                ...
                .send()
                .await
                .map_err(|e| anyhow!("Failed to send http request when downloading data: {}", e))?
                .bytes()
                .await
                .map_err(|e| anyhow!("Failed to read response: {}", e))?;
